...
| 4-bootstrap. kbu CLI | `kbu init-notebook` — per-project venv, util.py template, kernel registration, machine configs | 0.5 week |
...
- **Phase 4-bootstrap: kbu CLI replaces per-project sys.path setup**: Per-machine config in `KBUtilLib/machine_configs/`. `kbu init-notebook` creates a per-project venv via venvman, installs sibling repos as editable, renders `notebooks/util.py` from a Jinja template, registers a Jupyter kernel, and pins notebooks to it. The `bin/kbu` shell wrapper solves the bootstrap chicken-egg (KBUtilLib not yet installed) by setting PYTHONPATH before invocation.
