"""Vendored KING self-install compose/verify logic for KBUtilLib's ``kbu kind`` verbs.
...
def generate_serve_script(
    ...
    # Fail loudly rather than emit a wrapper that cannot launch. The wrapper
    # `exec`s this path, so if it does not exist the wrapper is dead -- but
    # nothing notices until the NEXT launch, which may be days later and will
    # not look like an install problem. Worse, on a host whose only running KING
    # was started by a PREVIOUS good wrapper, that process becomes the last one
    # able to run: stopping it strands the service, and boot-time autostart
    # (which launches only via this wrapper) cannot recover it either.
    if not king_serve.is_file():
        raise FileNotFoundError(
            f"KING launcher not found at {king_serve}. Refusing to write a "
            f"serve-kind.sh that cannot launch.\n"
            f"Resolved KING stack root: {king_stack_dir}\n"
            f"Set KING_STACK_DIR to the directory CONTAINING the `king` "
            f"checkout (laptop: ~/king-stack; BERDL pod: $HOME), or pass it "
            f"explicitly."
        )
