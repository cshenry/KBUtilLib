...
class MSBiochemUtilsImpl:
    ...
    def __init__(self, env, modelseed_db_path=None, auto_download=True):
        ...
        self.auto_download = auto_download
        self._biochem_db = None
        self._db_error: Optional[str] = None  # set when DB load fails
        self._identifier_hash = None
        self._structure_hash = None
        self._element_hashes = None
        self._rxn_identifier_hash = None
        ...
        # Attempt DB load; store error instead of raising so the object is
        # still constructable for capability discovery (registry WP3).
        # Any method that actually needs the DB will raise at call time.
        try:
            self._ensure_database_available()
    ...
    def available(self) -> bool:
        """Return True if the ModelSEED database loaded successfully."""
        return self._biochem_db is not None

    @property
    def unavailable_reason(self) -> Optional[str]:
        """Return the DB-load error message when unavailable, else None."""
    ...
    @capability(
        domain="biochem",
        summary="Search ModelSEED biochemistry compounds by identifier, structure, or formula.",
    ...
    @capability(
        domain="biochem",
        summary="Retrieve a ModelSEED compound object by its ID (e.g. 'cpd00001').",
        tags=("biochem", "lookup", "readonly"),
        visibility="public",
    )
    ...
    @capability(
        domain="biochem",
        summary="Retrieve a ModelSEED reaction object by its ID (e.g. 'rxn00001').",
        tags=("biochem", "lookup", "readonly"),
        visibility="public",
    )
...
# ---------------------------------------------------------------------------
# Attach pydantic schemas to the capability drafts on MSBiochemUtilsImpl.
#
# This runs once at module-import time (pydantic is always available).
# Keeping the import here (rather than inside the @capability call) keeps
# the decorator call itself concise and schema-free.
# ---------------------------------------------------------------------------

try:
    from ...core.capability import _CAPABILITY_ATTR
    from .schemas import (
    ...
    _drafts = {
        "search_compounds": (SearchCompoundsInput, SearchCompoundsOutput),
        "get_compound_by_id": (GetCompoundByIdInput, GetCompoundByIdOutput),
        "get_reaction_by_id": (GetReactionByIdInput, GetReactionByIdOutput),
    }
    for _method_name, (_in, _out) in _drafts.items():
        _fn = getattr(MSBiochemUtilsImpl, _method_name)
        _draft = getattr(_fn, _CAPABILITY_ATTR, None)
        if _draft is not None:
            _draft.input_model = _in
            _draft.output_model = _out

    del _drafts, _method_name, _fn, _draft, _in, _out  # noqa: F821
except Exception:
    # Pydantic or schema import failure must never crash the module.
    pass
