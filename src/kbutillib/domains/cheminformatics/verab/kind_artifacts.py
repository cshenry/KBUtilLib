"""KIND coscientist artifact emission for the verAB O-demethylation workflow.

``emit_kind_workflow`` writes a self-describing, reproducible run directory
that a KIND coscientist session (or any human collaborator) can use to
...
Layout of the emitted directory::
    ...
    <outdir>/
        ...
        prompt.md              # KIND coscientist prompt (kbu verab discover workflow)
...
def emit_kind_workflow(
    ...
    """Write a reproducible KIND coscientist input directory for the verAB workflow.
    ...
    prompt_content = (
        ...
        "directory was generated by `kbu verab emit-kind` and contains all inputs\n"
        "needed to reproduce or extend the analysis.\n\n"
        "**Note:** This directory contains *reproducible-run inputs* (seeds, rules,\n"
        "manifest) and a *ready-to-run prompt* — it is NOT itself a KIND bundle.\n"
        "To install the kbu verab KIND bundle, see `kbu kind install`.\n\n"
        "---\n\n"
        "## Goal\n\n"
        ...
        "```\n"
        "kbu verab emit-kind --outdir ./kind_run_verab --json\n"
        "```\n\n"
        "---\n\n"
        "## Expected output\n\n"
