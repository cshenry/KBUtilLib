...
from kbutillib.domains.thermo.thermo_predictors import (
...
from kbutillib.domains.thermo.thermo_predictors.base import BackendUnavailableError
...
def test_subpackage_imports_without_optional_deps():
    import kbutillib.domains.thermo.thermo_predictors as tp
...
def test_dgpredictor_unavailable_without_repo_and_never_fabricates(monkeypatch):
    ...
    monkeypatch.setattr(
        "kbutillib.domains.thermo.thermo_predictors.dgpredictor_backend._dependency_repo_path",
        lambda name: None,
    )
...
def test_molgpk_stub_is_unavailable_and_never_fabricates(monkeypatch):
    ...
    monkeypatch.setattr(
        "kbutillib.domains.thermo.thermo_predictors.molgpk_backend._dependency_repo_path",
        lambda name: None,
    )
    be = MolGPKBackend()
    assert be.available is False
...
def test_molgpk_compound_dgf_parses_worker_success(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kbutillib.domains.thermo.thermo_predictors.molgpk_backend.subprocess.run", _fake_run
    )
...
def test_compound_thermo_estimate_has_units_field():
    ...
    from kbutillib.domains.thermo.thermo_predictors.base import CompoundThermoEstimate
...
def test_reaction_thermo_estimate_has_units_field():
    ...
    from kbutillib.domains.thermo.thermo_predictors.base import ReactionThermoEstimate
